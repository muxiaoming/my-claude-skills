...
| **Reactor subscribeOn** | `Flux.subscribeOn(自定义Scheduler)` 切换线程时，Observation 未写入 Reactor Context，`Hooks.enableAutomaticContextPropagation()` 无值可传 | 虚拟线程上 Langfuse 多 TraceId |
...
5. **Reactor subscribeOn**：`Flux.subscribeOn(VT_SCHEDULER)` 切换虚拟线程后，该线程发出的所有 LLM 调用 TraceId 与请求入口 Span **完全相同**
6. **OTLP**：Langfuse 能看到完整 trace，无导出超时告警
